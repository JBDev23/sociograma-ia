...
export default function LandingPage() {
  ...
  const handleStartDemo = async () => {
    ...
    const timeoutId = setTimeout(() => {
      setLoadingMessage("Despertando el servidor (suele tardar ~40s)...");
    }, 3000);
  ...
  return (
    <div className="min-h-screen bg-brand-cream/30 font-sans selection:bg-brand-orange/20 selection:text-brand-orange overflow-hidden">
      ...
      <main className="max-w-[1200px] mx-auto px-6 pt-20 md:pt-32 pb-20 relative">
        ...
        <div className="flex flex-col items-center text-center max-w-[800px] mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <p className="text-lg md:text-xl text-brand-slate font-medium mb-10 leading-relaxed max-w-[600px]">
            Evalúa el clima social, detecta el acoso de forma temprana y deja
            que la Inteligencia Artificial organice las mesas de trabajo por ti
            en segundos.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
            <button 
              onClick={handleStartDemo}
              disabled={isLoading}
              className="w-full sm:w-auto px-8 py-4 bg-brand-orange text-white font-bold rounded-2xl hover:bg-[#e66a17] transition-all flex items-center justify-center gap-3 shadow-xl shadow-brand-orange/20 active:scale-[0.98] text-lg disabled:opacity-80 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <><Loader2 size={24} className="animate-spin" /> {loadingMessage}</>
              ) : (
            ...
            <button
              onClick={() =>
                alert("¡Aquí podrías poner un vídeo de cómo funciona la app!")
              }
      ...
      <section
        id="features"
        className="bg-white py-24 border-t border-brand-slate/10"
      >
        <div className="max-w-[1200px] mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-black text-brand-dark mb-4">
              Mucho más que encuestas
            </h2>
            <p className="text-brand-slate font-medium max-w-[600px] mx-auto">
              Todo lo que necesitas para entender las dinámicas de tu grupo y
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-brand-cream/30 p-8 rounded-[2rem] border border-brand-slate/15 hover:border-brand-orange/30 transition-colors group">
              ...
              <p className="text-brand-slate leading-relaxed font-medium text-sm">
                Visualiza las afinidades y conflictos en tiempo real. Filtra por
                contexto (trabajo u ocio) y descubre la estructura real de tu
                aula.
              </p>
            </div>
            ...
            <div className="bg-brand-cream/30 p-8 rounded-[2rem] border border-brand-slate/15 hover:border-brand-orange/30 transition-colors group">
              ...
              <p className="text-brand-slate leading-relaxed font-medium text-sm">
                Pide a la IA que genere grupos equitativos, que separe a los
                alumnos conflictivos o que integre a los aislados con un solo
                clic.
              </p>
            </div>
            ...
            <div className="bg-brand-cream/30 p-8 rounded-[2rem] border border-brand-slate/15 hover:border-brand-orange/30 transition-colors group">
              ...
              <p className="text-brand-slate leading-relaxed font-medium text-sm">
                Los informes automatizados detectan instantáneamente alumnos en
                riesgo de exclusión o posibles focos de acoso escolar.
              </p>
            </div>
          </div>
        </div>
